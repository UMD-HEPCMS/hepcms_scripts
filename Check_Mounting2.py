...
An additional email is sent to the admins in to admins2 if fixCVMFS.sh has been run in order to mount cvmfs.
...
if directory_mounted[3] == 0:
  ...
  time.sleep(30)
  msg2 = "/data/osg/scripts/fixCVMFS.sh has been run in order to mount cvmfs"
  o3 = subprocess.Popen("df -h", stdout=subprocess.PIPE, shell=True)
  o3_text = str(o3.communicate())
  usage_list = o3_text.split(r"\n")
  del usage_list[0]
...
#Sends email to admins2 if fixCVMFS.sh has been run in order to mount cvmfs
if msg2 != "" :
  msg2 = "On " + host + ": \n" + msg2
  from_addr = 'root@hepcms-hn.umd.edu'
  to_addr = ", ".join(admins2)
  subject = "WARNING: cvmfs mounting issues"
  header = "From: %s\nTo: %s\nSubject: %s\n" % (from_addr, to_addr, subject)
  email = header + msg2
  server = smtplib.SMTP('localhost')
  for addr in admins2:
