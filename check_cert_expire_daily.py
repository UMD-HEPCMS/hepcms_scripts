...
for i in range(0, len(GRID_SECURITY_DIRS)):
  ...
  #Checks which certificates will expire in 1-4 day(s); compiles a message containing the expiring certificates and their expiration dates
  for file in files:
    for x in range(1,5):
    ...
    if exp_date == "" :
     msg += "check_cert_expire_daily.py does not have access to %s%s, and its expiration date cannot be determined. \n" % (GRID_SECURITY_DIRS[i], file)
     break
    for y in range(1,5):
      if day["day%d" % y] == ["1"]: 
...
if msg:
  ...
  to_addr = ", ".join(admins)
  subject = "WARNING: Certificates will expire very soon." 
